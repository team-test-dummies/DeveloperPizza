    ...
    public void mapEndpoint(Javalin app) {
        ...
        app.post("/register", ctx -> {
            ...
            if (accountToRegister.getAccountType().length() == 0) {
                ctx.json(new Message("You must select an account type"));
                ctx.status(400);
            }
            else if (accountToRegister.getAccountName().length() == 0) {
                ctx.json(new Message("You must enter your full name"));
                ctx.status(400);
            }
            else if (accountToRegister.getUsername().length() == 0) {
                ctx.json(new Message("You must enter a username"));
                ctx.status(400);
            }
            else if (accountToRegister.getPassword().length() == 0) {
                ctx.json(new Message("You must enter a password"));
                ctx.status(400);
            }
            else if (accountToRegister.getEmail().length() == 0) {
                ctx.json(new Message("You must enter an email"));
                ctx.status(400);
            }
            else if (accountToRegister.getPhoneNumber().length() == 0) {
                ctx.json(new Message("You must enter a phone number"));
                ctx.status(400);
            }
            else if (accountToRegister.getLocation().length() == 0) {
                ctx.json(new Message("You must enter a location"));
                ctx.status(400);
            }else {
                try {
